 ...
 * </p>
 * <p>
 * Leap array use sliding window algorithm to count data. Each bucket cover {@code windowLengthInMs}
 * time span, and the total time span is {@link #intervalInMs}, so the total bucket amount is:
...
public abstract class LeapArray<T> {
  ...
  public LeapArray(int sampleCount, int intervalInMs) {
    ...
    AssertUtil.isTrue(intervalInMs > 0,
        "total time interval of the sliding window should be positive");
  ...
  /**
   * Get the bucket at current timestamp. 获取当前窗口的信息
   *
  ...
  /**
   * Check if a bucket is deprecated, which means that the bucket has been behind for at least an
   * entire window time span.
   *
  ...
  /**
   * Get aggregated value list for entire sliding window. The list will only contain value from
   * "valid" buckets.
   *
  ...
  /**
   * Get the valid "head" bucket of the sliding window for provided timestamp. Package-private for
   * test.
   *
   * @param timeMillis a valid timestamp in milliseconds
